...
import { HeaderUtils } from '@saman-core/data/lib/base/datasource/common/header.utils';
...
export class DatasourceConsumer {
  ...
  getByMethod<T>(
    ...
    return this._http.get<T>(
      `${this._protocol}://${this._url}/${method}`,
      {
  ...
  getPageByMethod<T>(
    ...
    return this._http.get<PageModel<T>>(
      `${this._protocol}://${this._url}/${method}`,
      {
        headers: HeaderUtils.generateHeaders(blockScreen, progressBar, ignoreError).headers,
  ...
  savePageMethod<T>(
    ...
    return this._http.post<PageModel<T>>(
      `${this._protocol}://${this._url}/${method}`,
      item,
      {
        headers: HeaderUtils.generateHeaders(blockScreen, progressBar, ignoreError).headers,
  ...
  updatePageMethod<T>(
    ...
    return this._http.put<PageModel<T>>(
      `${this._protocol}://${this._url}/${method}`,
      item,
      {
        headers: HeaderUtils.generateHeaders(blockScreen, progressBar, ignoreError).headers,
